...
- **Do-not**:
  ...
  - Never measure a branch against local `main` when the clone's local `main` can lag `origin/main` — the two integration refs drift whenever a sibling PR / retroactive squash-merges into origin mid-drive but local `main` is never fast-forwarded. A branch cut from the current `origin/main` then shows that already-merged commit as its *leading* commit relative to stale local `main`, so `piv_check` Check 1 ("plan is the branch's first commit") false-FAILs the gate (Night Shift #97 papered over it with a `git rebase --onto`). Fixed both ends: `piv_check.sh` subtracts the **more-advanced** of {local, `origin/`} `default` (whichever has the other as ancestor, drift-immune both directions), and `/implement` Phase 2 `git pull --ff-only`s a behind-local-`main` before cutting the worktree — so what you branch from == the real integration line == what the gate compares against. (retroactive: piv-check-base-ref-drift)
