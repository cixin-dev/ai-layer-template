...
- **Do-not**:
  ...
  - Never symlink Matt Pocock's `implement` skill into `~/.claude/skills/`, and never let a `setup-matt-pocock-skills` re-run add it back — this repo's `/implement` already covers its entire surface (TDD-at-seams → per-task checks → full gate at `/validate`) and **deliberately rejects its one extra move, the in-session `/code-review`**: review here is a fresh-session concern (Smart Zone — never review in the session that wrote the code), being encoded into `/validate` instead. The `/implement` menu collision is only the visible symptom; the reason is zero increment, not the name clash. (decision: #104)
