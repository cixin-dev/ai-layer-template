...
- **Do-not**:
  ...
  - Never search code with a bash `grep`/`rg` whose regex literally contains `| sh`, `| bash`, `$(`, or a backtick — the PreToolUse security guard string-matches the whole command (quoted pattern literals included) and false-blocks it as "opaque code execution" (it can't tell an executed `| bash` from those bytes inside a regex). Use the **Grep tool** for code search — the guard never inspects non-Bash tools; reserve a bash pipeline for `! …` when you truly intend one. (retroactive: search-via-grep-tool)
