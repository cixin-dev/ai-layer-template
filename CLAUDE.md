...
- **Do-not**:
  ...
  - Never write an **unconditional** operator-gated external side-effect into a PIV command step — a write to a pre-existing external object this session did **not** create (`gh issue comment`, `gh pr comment`, a label edit on someone's Issue). It's fine interactively (human approves at the prompt), but the headless auto-mode classifier denies it **by design**, and an unconditional instruction makes the agent escalate a benign, redundant no-op as **"Blocked — needs you"** into the Night Shift log. `/implement` Phase 4's courtesy `gh issue comment {N}` did exactly this on every NS run — yet the Issue↔PR link was already `/validate`'s `Closes #N`. Frame such a step **best-effort / context-aware**: on denial, silently skip — never a needs-you escalation. Never "fix" it by widening the permission dial (operator-only self-mod; weakens a real floor). (retroactive: courtesy-comment-headless-skip)
