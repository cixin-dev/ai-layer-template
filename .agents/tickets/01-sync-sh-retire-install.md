...
- **Never removes upstream symlinks.** An earlier design removed `to-issues` when `to-tickets`
  superseded it; ADR-0008 retired that fork and **keeps** `to-issues` (the unit of work is
  natively a GitHub Issue), so nothing is superseded — `sync.sh` carries no removal map and
  leaves every coexisting upstream symlink in place.
...
- **`--dry-run`** — prints exactly which links it would create, and mutates nothing. Mirror the
  retired `install.sh`'s contract ("print what would happen; change nothing") and its
  `[sync] would …` line style.
...
- [ ] A real run symlinks every owned skill/command into `~/.claude/{skills,commands}/`
      ...
      `grill-me`, `to-issues`) are untouched.
- [ ] Re-running `sync.sh` is a no-op against an already-synced state (idempotent).
- [ ] `sync.sh --dry-run` prints the planned create actions and changes nothing on disk.
...
- [ ] The test drives `sync.sh --dry-run` and asserts on its `[sync] would …` stdout, using
      **fixture directories** for the owned-items source and a fake `~/.claude` target — so the
      test is self-contained and needs no real `~/.claude`.
- [ ] Test cases cover: (a) fresh state — all owned items planned for linking; (b) idempotency
      — dry-run against an already-synced state plans no changes; (c) upstream safety —
      `grill-with-docs` / `to-prd` / `grill-me` / `to-issues` are never linked-over or removed
      (they are not owned names).
